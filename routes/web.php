...
Route::middleware(['auth', 'active.user'])->prefix('admin')->name('admin.')->group(function () {
    ...
    Route::delete('/teams/{team}/members/{member}', [TeamController::class, 'destroyMember'])->name('team.members.destroy');
    ...
    // This is for image view of attachment
    Route::get('/admin/task/attachment-view', [TaskController::class, 'viewAttachment'])->name('task.view-attachment');

    // AJAX route to get teams associated with the selected project
    Route::get('task/{project}/teams', [TaskController::class, 'getTeamsByProject'])->name('task.teams');

    // AJAX route to get employees associated with the selected team
    Route::get('task/{team}/employees', [TaskController::class, 'getEmployeesByTeam'])->name('task.employees');
    ...
    */
    ...
    // 3. TEAM REPORT ROUTE
    // ==========================================
    Route::get('report/team-report', [ReportController::class, 'teamreport'])->name('report.team-report');

    // ==========================================
    // 4. USER REPORT ROUTE
    // ==========================================
    Route::get('report/user-report', [ReportController::class, 'userreport'])->name('report.user-report');

    // ==========================================
    // 5. GENERAL SYSTEM REPORTS & HUB
    // ==========================================
